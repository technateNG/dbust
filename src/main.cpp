    ...
    dbust::Unit& unit)
{
    ...
    std::chrono::milliseconds time_diff = std::chrono::duration_cast<std::chrono::milliseconds>(
    ...
    if (time_diff.count() >= config.get_delay()) {
        ...
        if (unit.get_state() != dbust::Unit::State::DISCONNECTED_WITH_ERROR && unit.get_state() != dbust::Unit::State::DISCONNECTED) {
            ...
            const std::size_t req_size = req_type.length() + config.get_target().get_resource_path().length() + unit.get_path().length() + 19 + config.get_target().get_host().length() + 16 + config.get_user_agent().length() + 23;
...
int main(const int argc, const char* argv[])
{
    ...
    while (word_ptr < config.get_dictionary().size()) {
        ...
        for (dbust::Unit& unit : units) {
            if (unit.get_state() == dbust::Unit::State::SENDED && unit.get_poll_revents() & POLLIN) {
                char buff[13] { '\0' };
                unit.receive(buff, 12);
                dbust::Response response(buff);
                if (counter % one_percent == 0) {
                    std::cerr << "[*] " << '(' << counter / one_percent << "%) " << counter << '/' << config.get_dictionary().size() << '\n';
                }
                ...
                unit.get_state() != dbust::Unit::State::SENDED && unit.get_poll_revents() & POLLOUT) {
                pollout_branch(config, dns_result, word_ptr, ext_pointer, req_type, unit);
            } else {
                if (unit.get_timeout_tp().time_since_epoch().count()) {
                    std::chrono::duration<double> duration = std::chrono::system_clock::now() - unit.get_timeout_tp();
                    if (duration.count() > config.get_timeout()) {
                        unit.set_state(dbust::Unit::State::BROKEN);
                        std::cerr << "[!] Timeout hit for endpoint: " << unit.get_path() << " Reconnect issued." << std::endl;
                        unit.close();
                        unit.set_timeout_tp(std::chrono::system_clock::from_time_t(0));
                        int fd = dbust::get_socket(*dns_result);
                        unit.set_file_descriptor(fd);
